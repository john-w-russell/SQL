...
--         , nvl(buck_ship_to_account, fnoi_ship_to_account) as full_site
   ...
   , customer_orders as (
    ...
    select
        ...
        , fbk.net_amount
        , min(fbk.order_date) over (partition by soi.full_site)                 as first_order_at
        , min(iff(order_type = 'CR'
                  , fbk.order_date
                  , null)) over (partition by soi.full_site)                    as first_cr_ordered_at
        , max(iff(fbk.is_gko_order
                  , fbk.order_date
                  , null)) over (partition by soi.full_site)                    as last_gko_order_at
        , min(iff(order_type = 'EC'
                  , fbk.order_date
                  , null)) over (partition by soi.full_site)                    as first_ec_ordered_at
    from
        bookings                                                                fbk
