...
with total_spend as (
   ...
   , customer_orders as (
     ...
     */
    select
        soi.ship_attention                                                      as customer_name
        , soi.full_site
        , soi.company_name
        ...
        , mode(sfa.segment__c) over (partition by soi.company_name)             as segment
        ...
        , fbk.net_amount
        , min(fbk.order_date) over (partition by soi.ship_attention)            as first_order_at
        , min(iff(order_type = 'CR'
                  , fbk.order_date
                  , null)) over (partition by soi.ship_attention)               as first_cr_ordered_at
        , max(iff(fbk.is_gko_order
                  , fbk.order_date
                  , null)) over (partition by soi.ship_attention)               as last_gko_order_at
        , min(iff(order_type = 'EC'
                  , fbk.order_date
                  , null)) over (partition by soi.ship_attention)               as first_ec_ordered_at
    from
        bookings                                                                fbk
    left join sales_order_item_rollup                                           soi
        on fbk.buck_order_id = soi.buck_order_id
   ...
   , customer_spine as (
    select
        ...
        ||
          to_varchar(date_part(quarter
              , cust.first_order_at))                                           as customer_quarter_cohort
        , ord.segment
        , ord.industry
    ...
    from
        ...
        cross join (
            select distinct
                customer_daily_rollup.customer_name
        ...
        left join (
            ...
            and cust.customer_name = ord.customer_name
    where
   ...
   , upsell_tracking as (
    ...
    select
        customer_spine.dte
        , customer_spine.customer_name
        , coalesce(customer_spine.full_site, lag(customer_spine.full_site)
            ignore nulls over (partition by customer_spine.customer_name
                order by dte))                                                  as full_site
   ...
   , cumulative_upsells as (
    select
        ...
        , coalesce(upsell_tracking.is_upsell_target
            ...
            over (partition by upsell_tracking.customer_name
        ...
        , sum(iff(upsell_tracking.order_date is not null
            ...
            , upsell_tracking.num_orders
            , 0))
            over (partition by upsell_tracking.customer_name, yr
...
--         , cumulative_upsell_amt * 1.25                                          as target_amt
    ...
    , final as (
    select
        cumulative_upsells.customer_name
        , cumulative_upsells.full_site
        ...
        , cumulative_upsells.customer_quarter_cohort
